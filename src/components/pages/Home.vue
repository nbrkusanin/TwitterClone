<template>
  <div class="flex justify-start">
    ...
    <div class="w-1/2 border border-t-0 border-b-0 border-green-200 center-section">
      ...
      <div v-if="loggedUser" class="border-b border-green-200">
      ...
      <div>
        <div v-for="post in posts" :key="post.id" class="p-3 border-b border-green-200">
          ...
          <div v-if="loggedUser" class="w-full flex justify-end mt-3">
            <svg @click="editPostModal(post.id, post.title, post.post)" v-if="post.userId === user.id" class="fill-red w-6 h-6 mr-2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 64 64" id="Layer_1" version="1.1" viewBox="0 0 64 64" xml:space="preserve"><g><path d="M55.736,13.636l-4.368-4.362c-0.451-0.451-1.044-0.677-1.636-0.677c-0.592,0-1.184,0.225-1.635,0.676l-3.494,3.484   l7.639,7.626l3.494-3.483C56.639,15.998,56.639,14.535,55.736,13.636z"/><polygon points="21.922,35.396 29.562,43.023 50.607,22.017 42.967,14.39  "/><polygon points="20.273,37.028 18.642,46.28 27.913,44.654  "/><path d="M41.393,50.403H12.587V21.597h20.329l5.01-5H10.82c-1.779,0-3.234,1.455-3.234,3.234v32.339   c0,1.779,1.455,3.234,3.234,3.234h32.339c1.779,0,3.234-1.455,3.234-3.234V29.049l-5,4.991V50.403z"/></g></svg>
            <svg @click="deletePostModal(post.id)" v-if="post.userId === user.id" class="fill-gray w-6 h-6 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><path d="M12 38c0 2.21 1.79 4 4 4h16c2.21 0 4-1.79 4-4v-24h-24v24zm26-30h-7l-2-2h-10l-2 2h-7v4h28v-4z"/><path d="M0 0h48v48h-48z" fill="none"/></svg>
            <svg class="fill-green w-6 h-6 mr-5" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="Layer_1" style="enable-background:new 0 0 30 30;" version="1.1" viewBox="0 0 30 30" xml:space="preserve"><path d="M25,24H5c-1.105,0-2-0.895-2-2V7c0-1.105,0.895-2,2-2h20c1.105,0,2,0.895,2,2v15C27,23.105,26.105,24,25,24z"/><polygon points="9,28 9,23 15,23 10.818,28.57 "/><circle cx="10" cy="28" r="1"/></svg>
          </div>
        </div>
      </div>
    </div>
